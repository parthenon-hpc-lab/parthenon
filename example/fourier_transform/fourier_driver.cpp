...
void FillTestField(MeshBlock *pmb, ParameterInput *pin) {
  ...
  pmb->par_for(
      ...
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        field(k, j, i) = Kokkos::sin(coords.Xc<1>(i))
...
Packages_t ProcessPackages(std::unique_ptr<ParameterInput> &pin) {
  ...
  parthenon::Metadata m({parthenon::Metadata::Cell, parthenon::Metadata::Derived, parthenon::Metadata::OneCopy});
...
parthenon::DriverStatus FourierDriver::Execute() {
  ...
  parthenon::ParArray1D<std::complex<Real>> output("fft output", FFTManager->size_fourier_space_box());
  // also pre-allocate array for the recovered field after inverse FFT, to check round-trip accuracy:
  parthenon::ParArray1D<Real> input_recovered("fft input recovered", FFTManager->size_real_space_box());
  ...
  Kokkos::parallel_reduce("ComputeError",
    Kokkos::RangePolicy<>(0, FFTManager->size_real_space_box()),
...
#ifdef MPI_PARALLEL
  PARTHENON_MPI_CHECK(MPI_Allreduce(&local_max_error, &max_error, 1,
