...
TaskStatus WeightedSumDataAll(MeshData<Real> *in1, MeshData<Real> *in2, Real w1, Real w2,
                              MeshData<Real> *out) {
  ...
  constexpr auto cl = parthenon::CellLevel::same;
  {
    ...
    if (desc.nvar_tot > 0)
      WeightedSumData(cl, parthenon::TopologicalType::Cell, desc, in1, in2, w1, w2, out);
  }
  {
    static auto desc = parthenon::MakePackDescriptor<parthenon::variable_names::any>(
        pmesh->resolved_packages.get(),
        {parthenon::Metadata::Independent, parthenon::Metadata::Face}, {});
    if (desc.nvar_tot > 0)
      WeightedSumData(cl, parthenon::TopologicalType::Face, desc, in1, in2, w1, w2, out);
  }
  {
    static auto desc = parthenon::MakePackDescriptor<parthenon::variable_names::any>(
        pmesh->resolved_packages.get(),
        {parthenon::Metadata::Independent, parthenon::Metadata::Edge}, {});
    if (desc.nvar_tot > 0)
      WeightedSumData(cl, parthenon::TopologicalType::Edge, desc, in1, in2, w1, w2, out);
  }
  return TaskStatus::complete;
}
...
void StokesComponent(Real fac, parthenon::CellLevel cl,
  ...
  const int njtot =
      cellbounds.ncellsj(IndexDomain::entire, TeVar == TE::CC ? TE::CC : TE::NN);
  const int nitot =
      cellbounds.ncellsi(IndexDomain::entire, TeVar == TE::CC ? TE::CC : TE::NN);
  std::size_t scratch_size_in_bytes =
      parthenon::ScratchPad2D<Real>::shmem_size(njtot, nitot) * (2 + koff);

  parthenon::par_for_outer(
      PARTHENON_AUTO_LABEL, scratch_size_in_bytes, scratch_level, 0,
      pack_out.GetNBlocks() - 1, kb.s, kb.e,
      KOKKOS_LAMBDA(parthenon::team_mbr_t member, const int b, const int k) {
        auto &coords = pack_in.GetCoordinates(b);
        parthenon::Indexer2D idxer_entire({0, njtot - 1}, {0, nitot - 1});
        parthenon::ScratchPad2D<Real> Vflux(member.team_scratch(scratch_level), njtot,
                                            nitot);
        parthenon::ScratchPad2D<Real> Vvar(member.team_scratch(scratch_level), njtot,
                                           nitot);
        ...
        parthenon::ScratchPad2D<Real> Vfluxu(member.team_scratch(scratch_level),
                                             koff ? njtot : 0, koff ? nitot : 0);
        if (koff) {
          parthenon::par_for_inner(
              member, 0, idxer_entire.size() - 1, [&](const int idx) {
                const auto [j, i] = idxer_entire(idx);
        ...
        const int npoints =
            idxer_entire.GetFlatIdx(jb.e, ib.e) - idxer_entire.GetFlatIdx(jb.s, ib.s) + 1;
        for (int l = pack_out.GetLowerBound(b); l <= pack_out.GetUpperBound(b); ++l) {
          Real const *const flxl = &pack_in.flux(b, TeFlux, l, k, jb.s, ib.s);
          Real const *const flxu =
              &pack_in.flux(b, TeFlux, l, k + koff, jb.s + joff, ib.s + ioff);
          Real const *const vfl = &Vflux(jb.s, ib.s);
          Real const *const vfu =
              koff ? &Vfluxu(jb.s + joff, ib.s + ioff) : &Vflux(jb.s + joff, ib.s + ioff);
...
TaskStatus StokesAll(MeshData<Real> *in, MeshData<Real> *out) {
  ...
  constexpr auto cl = parthenon::CellLevel::same;
  {
    ...
    if (desc.nvar_tot > 0)
      Stokes(cl, parthenon::TopologicalType::Cell, desc, ndim, in, out);
  }
  {
    static auto desc = parthenon::MakePackDescriptor<parthenon::variable_names::any>(
        pmesh->resolved_packages.get(),
        {parthenon::Metadata::WithFluxes, parthenon::Metadata::Face},
        {parthenon::PDOpt::WithFluxes});
    if (desc.nvar_tot > 0)
      Stokes(cl, parthenon::TopologicalType::Face, desc, ndim, in, out);
  }
  {
    static auto desc = parthenon::MakePackDescriptor<parthenon::variable_names::any>(
        pmesh->resolved_packages.get(),
        {parthenon::Metadata::WithFluxes, parthenon::Metadata::Edge},
        {parthenon::PDOpt::WithFluxes});
    if (desc.nvar_tot > 0)
      Stokes(cl, parthenon::TopologicalType::Edge, desc, ndim, in, out);
  }
  return TaskStatus::complete;
}
