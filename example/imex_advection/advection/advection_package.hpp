...
// so.
//========================================================================================
#ifndef SRC_ADVECTION_PACKAGE_HPP_
#define SRC_ADVECTION_PACKAGE_HPP_
...
#include "../utilities/variable_macro.hpp"
...
inline TaskStatus ImplicitSourceUpdate(Real dt, MeshData<Real> *md_in,
  ...
  auto pack_in = desc.GetPack(md_in);
  auto pack_out = desc.GetPack(md_out);
...
inline TaskStatus Source(MeshData<Real> *md_in, MeshData<Real> *md_out) {
  using TE = parthenon::TopologicalElement;
...
TaskStatus CalculateVectorFluxes(parthenon::TopologicalElement edge,
  ...
  // edge in the quartants of the chosen edge
  TE fe;
  ...
  // 4. Add the diffusive piece to the numerical flux, which is proportional to
  // the curl
  ib = md->GetBoundsI(cl, IndexDomain::interior, edge);
  jb = md->GetBoundsJ(cl, IndexDomain::interior, edge);
  kb = md->GetBoundsK(cl, IndexDomain::interior, edge);
...
#endif // SRC_ADVECTION_PACKAGE_HPP_
