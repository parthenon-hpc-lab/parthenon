...
using View4D =  Kokkos::View<Real****, Kokkos::LayoutRight, Kokkos::DefaultExecutionSpace >;
...
double container_test_wrapper(const int n_burn, const int n_perf, 
    Container<Real>& container_in,Container<Real>& container_out) {
  ...
  return kernel_timer_wrapper(n_burn, n_perf, [&]() {
        par_for(
            ...
            KOKKOS_LAMBDA(const int l, const int k, const int j, const int i) {
              var_view_out(l, k, j, i) =
                  2.*var_view_in(l, k, j, i);
            });
      });
}
...
int main(int argc, char* argv[]) {
  Kokkos::initialize(argc, argv);
  {
    ...
    if( n_var%n_vector != 0 ){
      std::cerr<<"Error! n_var \% n_vector ="<< n_var%n_vector << " is not zero"<< std::endl;
      exit(0);
    }
    ...
    double time_view4d = kernel_timer_wrapper( n_run, n_run,
      [&] () {
        Kokkos::parallel_for( "View4D Loop", policy,
          KOKKOS_LAMBDA (const int &idx){
            ...
            view4d_out(v_var,k_grid,j_grid,i_grid) = 2.*view4d_in(v_var,k_grid,j_grid,i_grid);
    ...
    for ( int i = 0; i < n_var ; i++){
      container_scalars_in.Add(std::string("s_in")+std::to_string(i), m_in, scalar_block_size);
      container_scalars_out.Add(std::string("s_out")+std::to_string(i), m_out, scalar_block_size);
    }
    double time_scalars = container_test_wrapper(n_run,n_run,container_scalars_in,container_scalars_out);
    ...
    for ( int i = 0; i < n_var ; i+=n_vector){
      container_vectors_in.Add(std::string("v_in")+std::to_string(i), m_in, vector_block_size);
      container_vectors_out.Add(std::string("v_out")+std::to_string(i), m_out, vector_block_size);
    }
    double time_vectors = container_test_wrapper(n_run,n_run,container_vectors_in,container_vectors_out);

    //Time a container with a mix of vectors and scalars, every other one, scalars to backfill
    ...
    Container<Real> container_mix_out;
    {
      ...
      while( i < n_var){
        if(add_scalar){
          container_mix_in.Add(std::string("s_in")+std::to_string(i), m_in, scalar_block_size);
          container_mix_out.Add(std::string("s_out")+std::to_string(i), m_out, scalar_block_size);
          i++;
          if( i+n_vector <= n_var){//Add a vector if it will fit
            add_scalar = false;
          }
        } else {
          container_mix_in.Add(std::string("v_in")+std::to_string(i), m_in, vector_block_size);
          container_mix_out.Add(std::string("v_out")+std::to_string(i), m_out, vector_block_size);
          i+=n_vector;
        }
      }
    
    }
    double time_mix = container_test_wrapper(n_run,n_run,container_mix_in,container_mix_out);
    ...
    double time_one_vector = container_test_wrapper(n_run,n_run,container_one_vector_in,container_one_vector_out);
    ...
    double time_always_pack = container_always_pack_test_wrapper(n_run,n_run,container_always_pack_in,container_always_pack_out);



    double timings[] = {time_view4d,time_view_of_view3d,time_scalars,time_vectors,time_mix,time_one_vector,time_always_pack};


    std::cout<< n_var << " " << n_side << " " << n_run << " " << n_vector;
    for( double timing : timings){
      std::cout << " " << timing << " " << static_cast<double>(n_grid)*static_cast<double>(n_run)/timing;
    }
    std::cout << std::endl;
  }
  Kokkos::finalize();
