...
void PoissonDriver::AddMultiGridTasksLevel(TaskRegion &region, int level, int max_level) { 
  ...
  for (int i = 0; i < num_partitions; ++i) { 
    ...
    // 1. Copy residual from dual purpose communication field to the rhs, copy actual RHS for finest level
    std::string label = "Receiving residual on level " + std::to_string(level); 
    auto print_residual_recv = tl.AddTask(set_from_finer, PrintChosenValues<res_err>, md, label);
    ...
    if (level > 0) { 
      // 2. Do pre-smooth and fill solution on this level
      //auto pre_smooth = tl.AddTask(comm_rhs | build_matrix | zero_u, BlockLocalTriDiagX<u>, md); 
      ...
      for (int jacobi_iter = 0; jacobi_iter < jacobi_iterations / 2; ++jacobi_iter) {
        ...
        auto jacobi1 = tl.AddTask(comm1, JacobiIteration<u, res_err>, md, 1.0 - damping, level);
        auto comm2 = AddBoundaryExchangeTasks(jacobi1, tl, md, true);
        pre_previous_iter = tl.AddTask(comm2, JacobiIteration<res_err, u>, md, 1.0 - damping, level);
      }
      ...
      auto recv_from_coarser =
          tl.AddTask(communicate_to_coarse, ReceiveBoundBufs<BoundaryType::gmg_prolongate_recv>, md);
      ...
      auto prolongate = tl.AddTask(
          set_from_coarser | zero_res, ProlongateBounds<BoundaryType::gmg_prolongate_recv>, md);
      ...
      auto update_err = tl.AddTask(printout, AddFieldsAndStore<u, res_err, u>, md, 1.0, 1.0);
      auto comm_err = AddBoundaryExchangeTasks(update_err, tl, md, true);
      //std::string label3 = "Old solution vs. corrected solution" + std::to_string(level); 
      //auto printout3 = tl.AddTask(comm_err, PrintChosenValues<u, res_err>, md, label3);
      ...
      for (int jacobi_iter = 0; jacobi_iter < jacobi_iterations / 2; ++jacobi_iter) {
        ...
        auto jacobi1 = tl.AddTask(comm1, JacobiIteration<u, temp>, md, 1.0 - damping, level);
        auto comm2 = AddBoundaryExchangeTasks(jacobi1, tl, md, true);
        previous_iter = tl.AddTask(comm2, JacobiIteration<temp, u>, md, 1.0 - damping, level);
      }
      auto copy_over = tl.AddTask(previous_iter, AddFieldsAndStore<u, u, res_err>, md, 1.0, 0.0);
    ...
    // 9. Send communication field to next finer level (should be error field for that level)
    ...
    if (level < max_level) {
      ...
      auto update_solution = tl.AddTask(comm, AddFieldsAndStore<solution, res_err, solution>, md, 1.0, 1.0); 
