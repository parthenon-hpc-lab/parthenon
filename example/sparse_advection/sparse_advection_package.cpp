...
Real EstimateTimestepBlock(MeshBlockData<Real> *rc) {
  ...
  pmb->par_reduce(
      ...
      KOKKOS_LAMBDA(const int v, const int k, const int j, const int i, Real &lmin_dt) {
        if (vx[v] != 0.0)
          lmin_dt = std::min(lmin_dt, coords.cellWidth(X1DIR, k, j, i) / std::abs(vx[v]));
        if (vy[v] != 0.0)
          lmin_dt = std::min(lmin_dt, coords.cellWidth(X2DIR, k, j, i) / std::abs(vy[v]));
        if (vz[v] != 0.0)
          lmin_dt = std::min(lmin_dt, coords.cellWidth(X3DIR, k, j, i) / std::abs(vz[v]));
      },
