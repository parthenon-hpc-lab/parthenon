...
class PoissonEquation { 
 public:
  ...
  static parthenon::TaskStatus AxImpl(std::shared_ptr<parthenon::MeshData<Real>> &md_in,
    ...
    parthenon::par_for(
      "PoissonNodal::Ax", 0, pack_in.GetNBlocks() - 1, kb.s, kb.e, jb.s, jb.e, ib.s, ib.e,
      KOKKOS_LAMBDA(const int b, const int k, const int j, const int i) {
        ...
        Ax -= (pack_in(b, te, var_t(), k, j, i + 1) + pack_in(b, te, var_t(), k, j, i - 1)) / (dx1 * dx1); 
        if (ndim > 1) {
          Ax += pack_in(b, te, var_t(), k, j, i) * 2.0 / (dx2 * dx2);
          Ax -= (pack_in(b, te, var_t(), k, j + 1, i) + pack_in(b, te, var_t(), k, j - 1, i)) / (dx2 * dx2); 
        }
        if (ndim > 2) {
          Ax += pack_in(b, te, var_t(), k, j, i) * 2.0 / (dx3 * dx3);
          Ax -= (pack_in(b, te, var_t(), k + 1, j, i) + pack_in(b, te, var_t(), k - 1, j, i)) / (dx3 * dx3); 
        }
  ...
  static parthenon::TaskStatus SetBoundary(std::shared_ptr<parthenon::MeshData<Real>> &md) {
    ...
    parthenon::par_for(
      "PoissonNodal::SetBoundary", 0, pack.GetNBlocks() - 1, kb.s, kb.e, jb.s, jb.e, ib.s, ib.e,
      KOKKOS_LAMBDA(const int b, const int k, const int j, const int i) {
  ...
  parthenon::TaskStatus SetDiagonal(std::shared_ptr<parthenon::MeshData<Real>> &/*md_mat*/,
    ...
    parthenon::par_for(
        ...
        KOKKOS_LAMBDA(const int b, const int k, const int j, const int i) {
          ...
          pack_diag(b, te, var_t(), k, j, i) = alpha 
                                             + 2.0 / (dx1 * dx1) 
                                             + (ndim > 1) * 2.0 / (dx2 * dx2)
                                             + (ndim > 2) * 2.0 / (dx3 * dx3); 
        });
