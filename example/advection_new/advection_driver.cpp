...
TaskCollection AdvectionDriver::MakeTaskCollection(BlockList_t &blocks, const int stage) {
  ...
  for (int i = 0; i < num_partitions; i++) {
    ...
    auto flx_fine = none;
    for (auto face : faces) {
      flx = flx | tl.AddTask(none, advection_package::CalculateFluxes<pack_desc_t>, desc, face, parthenon::CellLevel::same, mc0.get());
      flx_fine = flx_fine | tl.AddTask(none, advection_package::CalculateFluxes<pack_desc_fine_t>, desc_fine, face, parthenon::CellLevel::fine, mc0.get());
    }

    auto set_flx = parthenon::AddFluxCorrectionTasks(start_flxcor | flx | flx_fine, tl, mc0, pmesh->multilevel);
    ...
    auto flux_div_fine = tl.AddTask(set_flx, Stokes<pack_desc_fine_t>, 
                                    parthenon::CellLevel::fine,
                                    parthenon::TopologicalType::Cell,
                                    desc_fine, pmesh->ndim, 
                                    mc0.get(), mdudt.get());
    ...
    auto avg_data_fine = tl.AddTask(flux_div_fine, WeightedSumData<pack_desc_fine_t>, parthenon::CellLevel::fine, 
        parthenon::TopologicalElement::CC, desc_fine, mc0.get(), mbase.get(), beta, 1.0 - beta, mc0.get());
    ...
    auto update_fine = tl.AddTask(avg_data_fine, WeightedSumData<pack_desc_fine_t>, parthenon::CellLevel::fine, 
        parthenon::TopologicalElement::CC, desc_fine, mc0.get(), mdudt.get(), 1.0, beta * dt, mc1.get());

    auto boundaries = parthenon::AddBoundaryExchangeTasks(update | update_fine | start_send, tl, mc1, pmesh->multilevel);
