...
RegionSize Tree::GetBlockDomain(LogicalLocation loc) const { 
  ...
  for (auto dir : {X1DIR, X2DIR, X3DIR}) { 
   if (!domain.symmetry(dir)) { 
     int n = 1 << loc.level(); 
     out.xmin(dir) = domain.LogicalToActualPosition(loc.l(dir - 1) / n, dir);
     out.xmax(dir) = domain.LogicalToActualPosition((loc.l(dir - 1) + 1) / n, dir);
   } 
   // If this is a translational symmetry direction, set the cell to cover the entire tree
   // in that direction. 
  }
  return out; 
}
...
Forest Forest::AthenaXX(RegionSize mesh_size, RegionSize block_size, std::array<bool, 3> periodic) { 
  ...
  for (int n = 0; n < idxer.size(); ++n) { 
    ...
    tree_domain.xmin(X1DIR) = mesh_size.LogicalToActualPosition(ix1 / ntree[0], X1DIR);
    tree_domain.xmax(X1DIR) = mesh_size.LogicalToActualPosition((ix1 + 1) / ntree[0], X1DIR);
    tree_domain.xmin(X2DIR) = mesh_size.LogicalToActualPosition(ix2 / ntree[1], X2DIR);
    tree_domain.xmax(X2DIR) = mesh_size.LogicalToActualPosition((ix2 + 1) / ntree[1], X2DIR);
    tree_domain.xmin(X3DIR) = mesh_size.LogicalToActualPosition(ix3 / ntree[2], X3DIR);
    tree_domain.xmax(X3DIR) = mesh_size.LogicalToActualPosition((ix3 + 1) / ntree[2], X3DIR);
    loc_tree.emplace_back(p_loc_tree_t{LogicalLocation(level, ix1, ix2, ix3), Tree::create(ndim, ref_level, tree_domain)});
  }
  ...
  for (int n = 0; n < idxer.size(); ++n) { 
    ...
    for (int o = 0; o < offsets.size(); ++o) { 
      ...
      if (add) { 
        ...
        loc_tree[n].second->AddNeighbor(loc_idx, loc_tree[neigh_idx].second, RelativeOrientation());
      }
    }
  }
  // Sort trees by their logical location in the tree mesh
  Forest fout; 
  std::sort(loc_tree.begin(), loc_tree.end(), [](const auto &a, const auto &b){return a.first < b.first;}); 
  for (auto &[loc, tree] : loc_tree) fout.trees.push_back(tree);
  
  // Assign tree ids 
  std::uint64_t id = 0;
  for (auto &tree : fout.trees) tree->SetId(id++); 
  return fout;
}
