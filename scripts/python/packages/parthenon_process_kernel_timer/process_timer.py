...
class func:
  ...
  def print_region(self):
    print(self.name, "time: " + str(self.total_time), "%wall: " + str(self.total_pct))
    for key,val in self.reg.items():
      if val[0] != self.total_time:
        print("  ", "type: " + self.reg_type[key], "line: " + key, "selftime: " + str(val[0]), "%func: " + str(100*val[0]/self.total_time))
    if self.total_time > 0:
      print("  ", "Kernel summary: Total kernel time: " + str(self.total_kernel_time), "  % Time in kernels: ", str(100*self.total_kernel_time/self.total_time))
    else:
...
def main(prof):
  ...
  if raw[0].rstrip() != "Regions:":
    print(prof + " does not appear to be a profile from the Kokkos simple kernel timer")
    print(raw[0])
    sys.exit(1)
  cline = 2
  in_regions = True
