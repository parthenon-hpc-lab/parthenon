...
##############################################################################
# Slightly adapted from https://github.com/Alpine-DAV/ascent/blob/0dedd70319145b3a31dd4d889fb82aaad995797b/scripts/build_ascent/build_ascent_cuda.sh
# Original Copyright (c) 2015-2023, Lawrence Livermore National Security, LLC.
...
conduit_install_dir=/usr/local/conduit-${conduit_version}/
...
vtkm_install_dir=/usr/local/vtk-m-${vtkm_version}/
...
cmake -S ${vtkm_src_dir} -B ${vtkm_build_dir} \
  ...
  -DVTKm_CUDA_Architecture=ampere \
...
camp_install_dir=/usr/local/camp-${camp_version}/
...
raja_install_dir=/usr/local/raja-${raja_version}/
...
umpire_install_dir=/usr/local/umpire-${umpire_version}/
...
mfem_install_dir=/usr/local/mfem-${mfem_version}/
...
ascent_install_dir=/usr/local/ascent-${ascent_version}/
