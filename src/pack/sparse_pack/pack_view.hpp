...
struct var_view_t {
 public:
  ...
  KOKKOS_FUNCTION
  parthenon::Real &operator()(int k, int j, int i) const { return var(k, j, i); }
};
...
struct pack_view_t {
  ...
  KOKKOS_INLINE_FUNCTION parthenon::Real &operator()(var_t v, Idxs &&...idxs) {
...
KOKKOS_INLINE_FUNCTION auto make_pack_view_impl(const sparse_pack_t &pack_in, const int b,
                                                const int s, TypeList<Ts...>) {
  using TL = TypeList<Ts...>;
  pack_view_t<Ts...> out;
  (
      [&] {
...
template <class... Ts>
KOKKOS_INLINE_FUNCTION auto make_pack_view(const SparsePack<Ts...> &pack_in,
                                           const int b) {
...
template <class... Ts>
KOKKOS_INLINE_FUNCTION auto make_sparse_pack_view(const SparsePack<Ts...> &pack_in,
