...
TaskStatus SendBoundBufs(std::shared_ptr<MeshData<Real>> &md) {
  ...
  if (rebuild) RebuildBufferCache<bound_type, true>(md, nbound, BndInfo::GetSendBndInfo, ProResInfo::GetSend);

  // Restrict
  auto pmb = md->GetBlockData(0)->GetBlockPointer();
  StateDescriptor *resolved_packages = pmb->resolved_packages.get();
  refinement::Restrict(resolved_packages, cache.prores_cache, pmb->cellbounds, pmb->c_cellbounds);
...
TaskStatus SetBounds(std::shared_ptr<MeshData<Real>> &md) {
  ...
  if (rebuild) RebuildBufferCache<bound_type, false>(md, nbound, BndInfo::GetSetBndInfo, ProResInfo::GetSet);
...
TaskStatus ProlongateBounds(std::shared_ptr<MeshData<Real>> &md) {
  ...
  if (nbound > 0 && pmesh->multilevel) {
    ...
    refinement::Prolongate(resolved_packages, cache.prores_cache, pmb->cellbounds, pmb->c_cellbounds);
