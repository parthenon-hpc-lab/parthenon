  ...
  for (int n = 0; n < neighbors.size(); n++) {
    ...
    if (nb.rank != Globals::my_rank) {
      ...
      if (send_size[nb.bufid] > 0) {
        // MPI sends only the active prefix of length send_size[bufid]. Mirror that exact
        // behavior for same-rank copies instead of copying the entire overallocated send
        // buffer. AMR can leave a sender with a larger historical buffer extent than the
        // number of particles being sent this cycle.
        if (ptarget_bswarm->bd_var_.recv[nb.targetid].extent(0) != send_size[nb.bufid]) {
