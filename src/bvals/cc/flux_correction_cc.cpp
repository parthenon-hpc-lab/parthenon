    ...
    if (nb.snb.level == pmb->loc.level - 1) {
      ...
      if (nb.fid == BoundaryFace::inner_x1 || nb.fid == BoundaryFace::outer_x1) {
        ...
        if (pmb->block_size.nx3 > 1) { // 3D
          ...
          pmb->par_for("SendFluxCorrection3D_x1", nl_, nu_, 0, ksize - 1, 0, jsize - 1,
                       KOKKOS_LAMBDA(const int nn, const int k, const int j) {
                         ...
                         sbuf(p) = (x1flx(nn, kf, jf, i) * amm +
                                    x1flx(nn, kf, jf + 1, i) * amp +
          ...
          pmb->par_for("SendFluxCorrection2D_x1", nl_, nu_, 0, jsize - 1,
                       KOKKOS_LAMBDA(const int nn, const int j) {
                         ...
                         sbuf(p) =
                             (x1flx(nn, k, jf, i) * am + x1flx(nn, k, jf + 1, i) * ap) /
                             tarea;
                       });
        } else { // 1D
          int k = kb.s, j = jb.s;
          psize = nu_ - nl_ + 1;
          pmb->par_for("SendFluxCorrection1D_x1", nl_, nu_, KOKKOS_LAMBDA(const int nn) {
        ...
        if (pmb->block_size.nx3 > 1) { // 3D
          ...
          pmb->par_for("SendFluxCorrection3D_x2", nl_, nu_, 0, ksize - 1, 0, isize - 1,
                       KOKKOS_LAMBDA(const int nn, const int k, const int i) {
                         ...
                         sbuf(p) = (x2flx(nn, kf, j, ii) * area00 +
                                    x2flx(nn, kf, j, ii + 1) * area01 +
          ...
          pmb->par_for("SendFluxCorrection2D_x2", nl_, nu_, 0, isize - 1,
                       KOKKOS_LAMBDA(const int nn, const int i) {
                         ...
                         sbuf(p) = (x2flx(nn, k, j, ii) * area0 +
                                    x2flx(nn, k, j, ii + 1) * area1) /
                                   tarea;
                       });
        }
        ...
        pmb->par_for("SendFluxCorrection3D_x3", nl_, nu_, 0, jsize - 1, 0, isize - 1,
                     KOKKOS_LAMBDA(const int nn, const int j, const int i) {
                       ...
                       sbuf(p) = (x3flx(nn, k, jf, ii) * area00 +
                                  x3flx(nn, k, jf, ii + 1) * area01 +
  ...
  for (int n = 0; n < pmb->pbval->nneighbor; n++) {
    ...
    if (nb.snb.level == pmb->loc.level + 1) {
      ...
      if (nb.fid == BoundaryFace::inner_x1 || nb.fid == BoundaryFace::outer_x1) {
        ...
        int ksize = ku - kl + 1;
        auto &x1flx = x1flux;
        pmb->par_for("ReceiveFluxCorrection_x1", nl_, nu_, kl, ku, jl, ju,
                     KOKKOS_LAMBDA(const int nn, const int k, const int j) {
        ...
        int isize = iu - il + 1;
        auto &x2flx = x2flux;
        pmb->par_for("ReceiveFluxCorrection_x2", nl_, nu_, kl, ku, il, iu,
                     KOKKOS_LAMBDA(const int nn, const int k, const int i) {
        ...
        int isize = iu - il + 1;
        auto &x3flx = x3flux;
        pmb->par_for("ReceiveFluxCorrection_x1", nl_, nu_, jl, ju, il, iu,
                     KOKKOS_LAMBDA(const int nn, const int j, const int i) {
