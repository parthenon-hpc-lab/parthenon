...
TaskStatus SendBoundaryBuffers(std::shared_ptr<MeshData<Real>> &md) {
  ...
  for (size_t num_it = 0; num_it < 5; num_it++) {
  Kokkos::parallel_for(
      "SendBoundaryBuffers TeamThread+ThreadVector",
  ...
  Kokkos::parallel_for(
      "SendBoundaryBuffers TeamThreadNv+ThreadVector",
      Kokkos::TeamPolicy<>(parthenon::DevExecSpace(), buffers_used, Kokkos::AUTO),
      KOKKOS_LAMBDA(parthenon::team_mbr_t team_member) {
        const int b = team_member.league_rank();
        const int &si = boundary_info(b).si;
      ...
      KOKKOS_LAMBDA(parthenon::team_mbr_t team_member) {
        ...
        Kokkos::parallel_for(
            Kokkos::TeamThreadRange<>(team_member, NkNjNi), [&](const int idx) {
                                     ...
                                     k += sk;
                                     j += sj;
              Kokkos::parallel_for(Kokkos::ThreadVectorRange(team_member, Nv),
                                   [&](const int v) {
                                     boundary_info(b).buf(idx + v * NkNjNi) =
                                         boundary_info(b).var(v, k, j, i);
                                   });
  ...
  Kokkos::parallel_for(
      "SendBoundaryBuffers TeamThread+for",
      Kokkos::TeamPolicy<>(parthenon::DevExecSpace(), buffers_used, Kokkos::AUTO),
      KOKKOS_LAMBDA(parthenon::team_mbr_t team_member) {
        const int b = team_member.league_rank();
        const int &si = boundary_info(b).si;
  ...
  Kokkos::parallel_for(
      "SendBoundaryBuffers TeamThread",
      Kokkos::TeamPolicy<>(parthenon::DevExecSpace(), buffers_used, Kokkos::AUTO),
      KOKKOS_LAMBDA(parthenon::team_mbr_t team_member) {
        const int b = team_member.league_rank();
        const int &si = boundary_info(b).si;
  ...
  Kokkos::parallel_for(
      "SendBoundaryBuffers TeamThread (idx)",
      Kokkos::TeamPolicy<>(parthenon::DevExecSpace(), buffers_used, Kokkos::AUTO),
      KOKKOS_LAMBDA(parthenon::team_mbr_t team_member) {
        const int b = team_member.league_rank();
        const int &si = boundary_info(b).si;
  ...
  Kokkos::parallel_for(
      "SendBoundaryBuffers TeamVector",
      Kokkos::TeamPolicy<>(parthenon::DevExecSpace(), buffers_used, Kokkos::AUTO),
      KOKKOS_LAMBDA(parthenon::team_mbr_t team_member) {
        const int b = team_member.league_rank();
        const int &si = boundary_info(b).si;
        ...
        const int NjNi = Nj * Ni;
        Kokkos::parallel_for(
            Kokkos::TeamVectorRange<>(team_member, NvNkNjNi), [&](const int idx) {
              const int v = idx / NkNjNi;
              int k = (idx - v * NkNjNi) / NjNi;
              int j = (idx - v * NkNjNi - k * NjNi) / Ni;
              int i = (idx - v * NkNjNi - k * NjNi - j * Ni) + si;
...
TaskStatus SetBoundaries(std::shared_ptr<MeshData<Real>> &md) {
  ...
  Kokkos::parallel_for(
      "SetBoundaries TeamThread+ThreadVector",
  ...
  Kokkos::parallel_for(
      "SetBoundaries TeamThreadNv+ThreadVector",
      Kokkos::TeamPolicy<>(parthenon::DevExecSpace(), boundary_info.extent(0),
                           Kokkos::AUTO),
      KOKKOS_LAMBDA(parthenon::team_mbr_t team_member) {
        const int b = team_member.league_rank();
  ...
  Kokkos::parallel_for(
      "SetBoundaries TeamThread+ThreadVectorNv",
      Kokkos::TeamPolicy<>(parthenon::DevExecSpace(), boundary_info.extent(0),
                           Kokkos::AUTO),
      KOKKOS_LAMBDA(parthenon::team_mbr_t team_member) {
        const int b = team_member.league_rank();
