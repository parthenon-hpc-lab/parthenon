...
namespace {
  ...
  void BuildBoundaryBufferSubset(std::shared_ptr<MeshData<Real>> &md, Mesh::comm_buf_map_t &buf_map) {
    ...
    ForEachBoundary<BTYPE>(md, [&](sp_mb_t pmb, sp_mbd_t /*rc*/, nb_t &nb, const sp_cv_t v) {
      ...
      if constexpr (BTYPE == BoundaryType::fluxcor_send || BTYPE == BoundaryType::fluxcor_recv)
          comm_label += "_flcor";
...
TaskStatus BuildBoundaryBuffers(std::shared_ptr<MeshData<Real>> &md) {
  ...
  BuildBoundaryBufferSubset<BoundaryType::flxcor_send>(md, pmesh->boundary_comm_flxcor_map);
  BuildBoundaryBufferSubset<BoundaryType::flxcor_recv>(md, pmesh->boundary_comm_flxcor_map);
