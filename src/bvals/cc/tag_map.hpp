...
// Struct uniquely identifying a particular geometric element (i.e. face, edge, or node) of a particular block
struct BlockGeometricElementId {
  int gid;
  int orientation;  
};
// Need to define (somewhat arbitrarily) comparison operators for the BlockGeometricElement 
// since we want the elements that coincide across to blocks across ranks to have a natural
// ordering so they can be placed in a map (which is ordered)
...
// map we want it to have the same value no matter which order the two BlockGeometricelementIds 
...
// Need to also define an ordering for an UnorderedPair, once again this choice is somewhat 
// arbitrary 
template<class T> 
...
class TagMap { 
  ...
  // Given the two blocks (one described by the MeshBlock and the other described by the firsts 
  // NeighborBlock information) return an ordered pair of BlockGeometricElementIds corresponding 
  // to the two blocks geometric elements that coincide. This serves as a unique key defining the 
  // two-way communication channel between these elements  
 ...
 public:   
  ...
  // iterate through a map for a given rank pair (which is already ordered by key because of the properties of st::map)
  // and assign each key a unique tag. By construction, this tag is consistent across all ranks. 
  void ResolveMap();
  
  // After the map has been resolved, get the tag for a particular MeshBlock NeighborBlock pair 
