          ...
          for (auto nox1 : xidxs) {
            for (auto nox2 : yidxs) {
              for (auto nox3 : zidxs) {
                ...
                nb.SetNeighbor(ranklist[nid], pleaf->GetLocation().level, nid, i, j, k,
nc, nid - nslist[ranklist[nid]], bufid, tbid, ifi[0], ifi[1]);
  ...
  // Now that we have found all of the neighbors, we can determine ownership of all of the
