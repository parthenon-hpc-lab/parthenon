...
void GetTopologicalElementOwnership(MeshBlockTree &tree, LogicalLocation owner_loc) {
  ...
  for (int i = -1; i <=1; ++i) {
    for (int j = -1; j <=1; ++j) {
      for (int k = -1; k <=1; ++k) {
        owns[i + 1][j + 1][k + 1] = true; 
        if (i==0 && j==0 && k==0) continue;
        auto pneighbor_tree = tree.FindNeighbor(owner_loc, i, j, k); 
        if (pneighbor_tree == nullptr) { 
          // I think this means there is no neighbor
          owns[i + 1][j + 1][k + 1] = true;
        } else if (!pneighbor_tree->IsLeaf()) { 
          // Same level neighbor block is refined, so the refined blocks 
          // must own this element 
          owns[i + 1][j + 1][k + 1] = false; 
        } else if (pneighbor_tree->GetLocation().level < owner_loc.level) { 
          // Neighbor is at a coarser level so this block must own 
          // this element
          owns[i + 1][j + 1][k + 1] = true; 
        } else { 
          // Blocks must be same level, and in the default model neither 
          // has ownership of the shared element
          owns[i + 1][j + 1][k + 1] = false; 
        }
      }
    }
...
void GetNeighborLocationList(MeshBlockTree &tree, LogicalLocation center_loc) {
  std::unordered_map<LogicalLocation, int> neighbor_map; 
  const ind ndim = 3;
  for (int i = -(ndim > 0); i <= (ndim > 0); ++i) {
    for (int j = -(ndim > 1); j <= (ndim > 1); ++j) {
      for (int k = -(ndim > 2); k <= (ndim > 2); ++k) {
        ...
        const int bufid = -1; // I don't think these are actually used, need to test
        const int tbid = -1; // I don't think these are actually used, need to test
        ...
        if (pneighbor_tree == nullptr) { 
          ...
          for (auto nox1 : xidxs) {
            for (auto nox2 : yidxs) {
              for (auto nox3 : zidxs) {
                ...
                nb.SetNeighbor(ranklist[nid], pleaf->GetLocation().level, nid, i, j, k, nc,  
                               nid - nslist[ranklist[nid]], bufid, tbid, ifi[0], ifi[1]);

                neighbor_map[pleaf->GetLocation()] = pleaf->GetGid();
              }
            }
          }
        } else { 
          ...
          if (neighbor_map.count(nloc) == 0) 
              neighbor_map[nloc] = pneighbor_tree->GetGid();
        }
      }
    }
  }
  // Now that we have found all of the neighbors, we can determine ownership of all of the topological elements
  // Maybe best to make this a map from topological element to sets of LocationIDs
