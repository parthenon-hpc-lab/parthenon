...
private:
  bool IsValid(const weak_t &in) const
  {
    return inuse_.count(in.key_);
  }
...
public:
  ...
  weak_t(Ts &&...args) : T(std::forward<Ts>(args)...), key_(default_key_){};
  ...
  inline bool IsValid()
  {
    if (key_ == default_key_ || pool_ == nullptr)
      return false;
