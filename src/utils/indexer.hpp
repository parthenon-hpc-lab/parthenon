...
auto MakeIndexerInt(Ts&&... args) {
  static_assert(sizeof...(Ts) % 2 == 0, "Must have an upper and lower end to each index range.");
  return impl::MakeIndexerIntImpl(std::array<int, sizeof...(Ts)>{args...}, std::make_index_sequence<sizeof...(Ts) / 2>());
}
