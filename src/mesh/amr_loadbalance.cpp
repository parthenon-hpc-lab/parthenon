...
void Mesh::PrepareSendCoarseToFineAMR(MeshBlock* pb, Real *sendbuf,
  ...
  if (ox2 == 0) jl = pb->js - isAtLeast2D,              ju = pb->js + pb->block_size.nx2/2;
  else        jl = pb->js + pb->block_size.nx2/2 - isAtLeast2D, ju = pb->je + isAtLeast2D;
  if (ox3 == 0) kl = pb->ks - is3D,              ku = pb->ks + pb->block_size.nx3/2;
  else        kl = pb->ks + pb->block_size.nx3/2 - is3D, ku = pb->ke + is3D;
  int p = 0;
