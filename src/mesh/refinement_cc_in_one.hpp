...
void Restrict(const Pack &fine, Pack &coarse,
              ParArray1D<RefinementInfo> &info) {
  ...
  if (fine.GetDim(3) > 1) { // 3D
    par_for_outer(
        ...
        DevExecSpace(), scratch_size_in_bytes, scratch_level, 0, nblocks - 1, 0,
        nvars - 1,
        KOKKOS_LAMBDA(team_mbr_t team_member, const int b, const int n) {
          par_for_inner(
              inner_loop_pattern_ttr_tag, team_member, info(b).sk, info(b).ek,
              info(b).sj, info(b).ej, info(b).si, info(b).ei,
              [&](const int ck, const int cj, const int ci) {
                ...
                // symmetry
                coarse(b, n, ck, cj, ci) =
                    (((fine(b, n, k, j, i) * vol000 +
                       fine(b, n, k, j + 1, i) * vol010) +
    ...
    par_for_outer(
        ...
        KOKKOS_LAMBDA(team_mbr_t team_member, const int b, const int n) {
          ...
          par_for_inner(
              inner_loop_pattern_ttr_tag, team_member, info(b).sj, info(b).ej,
              info(b).si, info(b).ei, [&](const int cj, const int ci) {
                const int j = (cj - info(b).cjb.s) * 2 + info(b).jb.s;
                const int i = (ci - info(b).cib.s) * 2 + info(b).ib.s;
                // KGF: add the off-centered quantities first to preserve FP
                // symmetry
                ...
                // symmetry
                coarse(b, n, 0, cj, ci) =
                    ((fine(b, n, 0, j, i) * vol00 +
                      fine(b, n, 0, j + 1, i) * vol10) +
    ...
    par_for_outer(
        ...
        KOKKOS_LAMBDA(team_mbr_t team_member, const int b, const int n) {
          ...
          par_for_inner(inner_loop_pattern_tvr_tag, team_member, info(b).si,
                        info(b).ei, [&](const int ci) {
                          const int i = (ci - info(b).cib.s) * 2 + info(b).ib.s;
