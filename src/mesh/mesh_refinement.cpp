...
void MeshRefinement::RestrictCellCenteredValues(
  ...
  auto &pco = pmb->pcoord;
  int si = (csi - pmb->active_coarse_cells.x.at(0).s)*2 + pmb->active_cells.x.at(0).s, ei = (cei - pmb->active_coarse_cells.x.at(0).s)*2 + pmb->active_cells.x.at(0).s + 1;

  // store the restricted data in the prolongation buffer for later use
  if (pmb->block_size.nx3>1) { // 3D
    for (int n=sn; n<=en; ++n) {
      for (int ck=csk; ck<=cek; ck++) {
        int k = (ck - pmb->active_coarse_cells.x.at(2).s)*2 + pmb->active_cells.x.at(2).s;
        for (int cj=csj; cj<=cej; cj++) {
          int j = (cj - pmb->active_coarse_cells.x.at(1).s)*2 + pmb->active_cells.x.at(1).s;
          ...
          for (int ci=csi; ci<=cei; ci++) {
            int i = (ci - pmb->active_coarse_cells.x.at(0).s)*2 + pmb->active_cells.x.at(0).s;
    ...
    int j = pmb->active_cells.x.at(1).s, cj = pmb->active_coarse_cells.x.at(1).s, k = pmb->active_cells.x.at(2).s, ck = pmb->active_coarse_cells.x.at(2).s;
    for (int n=sn; n<=en; ++n) {
      pco->CellVolume(k,j,si,ei,fvol_[0][0]);
      for (int ci=csi; ci<=cei; ci++) {
        int i = (ci - pmb->active_coarse_cells.x.at(0).s)*2 + pmb->active_cells.x.at(0).s;
...
void MeshRefinement::RestrictFieldX1(
  ...
  MeshBlock *pmb = pmy_block_;
  auto &pco = pmb->pcoord;
  int si = (csi - pmb->active_coarse_cells.x.at(0).s)*2 + pmb->active_cells.x.at(0).s, ei = (cei - pmb->active_coarse_cells.x.at(0).s)*2 + pmb->active_cells.x.at(0).s;
  ...
  if (pmb->block_size.nx3>1) { // 3D
    ...
    for (int ci=csi; ci<=cei; ci++) {
      int i = (ci - pmb->active_coarse_cells.x.at(0).s)*2 + pmb->active_cells.x.at(0).s;
      coarse(csk,csj,ci) = fine(pmb->active_cells.x.at(2).s,pmb->active_cells.x.at(1).s,i);
    }
  }
  ...
  if (pmb->block_size.nx3>1) { // 3D
    ...
    for (int cj=csj; cj<=cej; cj++) {
      ...
      for (int ci=csi; ci<=cei; ci++) {
        int i = (ci - pmb->active_coarse_cells.x.at(0).s)*2 + pmb->active_cells.x.at(0).s;
        Real tarea = sarea_x2_[0][0](i) + sarea_x2_[0][0](i+1);
        coarse(pmb->active_coarse_cells.x.at(2).s,cj,ci) =
    ...
    for (int ci=csi; ci<=cei; ci++) {
      int i = (ci - pmb->active_coarse_cells.x.at(0).s)*2 + pmb->active_cells.x.at(0).s;
      Real tarea = sarea_x2_[0][0](i) + sarea_x2_[0][0](i+1);
      coarse(pmb->active_coarse_cells.x.at(2).s,pmb->active_coarse_cells.x.at(1).s,ci) =
          (fine(k,j,i)*sarea_x2_[0][0](i) + fine(k,j,i+1)*sarea_x2_[0][0](i+1))/tarea;
    }
  }
...
void MeshRefinement::ProlongateCellCenteredValues(
  ...
  if (pmb->block_size.nx3 > 1) {
    for (int n=sn; n<=en; n++) {
      for (int k=sk; k<=ek; k++) {
        int fk = (k - pmb->active_coarse_cells.x.at(2).s)*2 + pmb->active_cells.x.at(2).s;
        ...
        Real dx3fp =  fx3p - x3c;
        for (int j = sj; j<=ej; j++) {
          int fj = (j - pmb->active_coarse_cells.x.at(1).s)*2 + pmb->active_cells.x.at(1).s;
          ...
          Real dx2fp = fx2p - x2c;
          for (int i=si; i<=ei; i++) {
            int fi = (i - pmb->active_coarse_cells.x.at(0).s)*2 + pmb->active_cells.x.at(0).s;
    ...
    int k = pmb->active_coarse_cells.x.at(2).s, fk = pmb->active_cells.x.at(2).s;
    for (int n=sn; n<=en; n++) {
      for (int j=sj; j<=ej; j++) {
        int fj = (j - pmb->active_coarse_cells.x.at(1).s)*2 + pmb->active_cells.x.at(1).s;
        const Real& x2m = pcoarsec->x2v(j-1);
    ...
    int k = pmb->active_coarse_cells.x.at(2).s, fk = pmb->active_cells.x.at(2).s, j = pmb->active_coarse_cells.x.at(1).s, fj = pmb->active_cells.x.at(1).s;
    for (int n=sn; n<=en; n++) {
      for (int i=si; i<=ei; i++) {
        int fi = (i - pmb->active_coarse_cells.x.at(0).s)*2 + pmb->active_cells.x.at(0).s;
        const Real& x1m = pcoarsec->x1v(i-1);
...
void MeshRefinement::ProlongateInternalField(
  ...
  MeshBlock *pmb = pmy_block_;
  auto &pco = pmb->pcoord;
  int fsi = (si - pmb->active_coarse_cells.x.at(0).s)*2 + pmb->active_cells.x.at(0).s, fei = (ei - pmb->active_coarse_cells.x.at(0).s)*2 + pmb->active_cells.x.at(0).s + 1;
  if (pmb->block_size.nx3 > 1) {
    for (int k=sk; k<=ek; k++) {
      int fk = (k - pmb->active_coarse_cells.x.at(2).s)*2 + pmb->active_cells.x.at(2).s;
      for (int j=sj; j<=ej; j++) {
...
#pragma unroll
    ...
    int fk = pmb->active_cells.x.at(2).s;
    for (int j=sj; j<=ej; j++) {
      int fj = (j - pmb->active_coarse_cells.x.at(1).s)*2 + pmb->active_cells.x.at(1).s;
      pco->Face1Area(fk,   fj,   fsi, fei+1, sarea_x1_[0][0]);
      pco->Face1Area(fk,   fj+1, fsi, fei+1, sarea_x1_[0][1]);
