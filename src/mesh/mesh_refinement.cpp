  ...
  if (pmb->block_size.nx3 > 1) { // 3D
    for (int ck = csk; ck <= cek; ck++) {
      ...
      for (int cj = csj; cj <= cej; cj++) {
        ...
        for (int ci = csi; ci <= cei; ci++) {
          ...
          const Real area00 = coords.da(X1DIR, k, j, i);
          const Real area01 = coords.da(X1DIR, k, j + 1, i);
          const Real area10 = coords.da(X1DIR, k + 1, j, i);
          const Real area11 = coords.da(X1DIR, k + 1, j + 1, i);
    ...
    for (int cj = csj; cj <= cej; cj++) {
      ...
      for (int ci = csi; ci <= cei; ci++) {
        ...
        const Real area0 = coords.da(X1DIR, k, j, i);
        const Real area1 = coords.da(X1DIR, k, j + 1, i);
...
void MeshRefinement::RestrictFieldX2(const ParArrayND<Real> &fine,
  ...
  if (pmb->block_size.nx3 > 1) { // 3D
    for (int ck = csk; ck <= cek; ck++) {
      ...
      for (int cj = csj; cj <= cej; cj++) {
        ...
        for (int ci = csi; ci <= cei; ci++) {
          int i =
              (ci - pmb->c_cellbounds.is(interior)) * 2 + pmb->cellbounds.is(interior);
          const Real area00 = coords.da(X2DIR, k, j, i);
          const Real area01 = coords.da(X2DIR, k, j, i + 1);
          const Real area10 = coords.da(X2DIR, k + 1, j, i);
          const Real area11 = coords.da(X2DIR, k + 1, j, i + 1);
          const Real tarea = area00 + area01 + area10 + area11;
          coarse(ck, cj, ci) =
    ...
    for (int cj = csj; cj <= cej; cj++) {
      ...
      for (int ci = csi; ci <= cei; ci++) {
        int i = (ci - pmb->c_cellbounds.is(interior)) * 2 + pmb->cellbounds.is(interior);
        const Real area0 = coords.da(X2DIR, k, j, i);
        const Real area1 = coords.da(X2DIR, k, j, i + 1);
        const Real tarea = area0 + area1;
...
void MeshRefinement::RestrictFieldX3(const ParArrayND<Real> &fine,
  ...
  if (pmb->block_size.nx3 > 1) { // 3D
    for (int ck = csk; ck <= cek; ck++) {
      ...
      for (int cj = csj; cj <= cej; cj++) {
        ...
        for (int ci = csi; ci <= cei; ci++) {
          int i =
              (ci - pmb->c_cellbounds.is(interior)) * 2 + pmb->cellbounds.is(interior);
          const Real area00 = coords.da(X3DIR, k, j, i);
          const Real area01 = coords.da(X3DIR, k, j, i + 1);
          const Real area10 = coords.da(X3DIR, k, j + 1, i);
          const Real area11 = coords.da(X3DIR, k, j + 1, i + 1);
          const Real tarea = area00 + area01 + area10 + area11;
          coarse(ck, cj, ci) =
              (fine(k, j, i) * area00 + fine(k, j, i + 1) * area01 +
    ...
    for (int ci = csi; ci <= cei; ci++) {
      int i = (ci - pmb->c_cellbounds.is(interior)) * 2 + pmb->cellbounds.is(interior);
      const Real area0 = coords.da(X3DIR, k, j, i);
      const Real area1 = coords.da(X3DIR, k, j, i + 1);
      const Real tarea = area0 + area1;
      coarse(pmb->c_cellbounds.ks(interior), pmb->c_cellbounds.js(interior), ci) =
          (fine(k, j, i) * area0 + fine(k, j, i + 1) * area1) / tarea;
    }
