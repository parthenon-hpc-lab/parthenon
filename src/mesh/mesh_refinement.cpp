...
void MeshRefinement::RestrictCellCenteredValues(
  ...
  auto &pco = pmb->pcoord;
  int si = (csi - pmb->c_cells.x1s(interior))*2 + pmb->cells.x1s(interior), ei = (cei - pmb->c_cells.x1s(interior))*2 + pmb->cells.x1s(interior) + 1;

  // store the restricted data in the prolongation buffer for later use
  if (pmb->block_size.nx3>1) { // 3D
    for (int n=sn; n<=en; ++n) {
      for (int ck=csk; ck<=cek; ck++) {
        int k = (ck - pmb->c_cells.x3s(interior))*2 + pmb->cells.x3s(interior);
        for (int cj=csj; cj<=cej; cj++) {
          int j = (cj - pmb->c_cells.x2s(interior))*2 + pmb->cells.x2s(interior);
          ...
          for (int ci=csi; ci<=cei; ci++) {
            int i = (ci - pmb->c_cells.x1s(interior))*2 + pmb->cells.x1s(interior);
    ...
    int j = pmb->cells.x2s(interior), cj = pmb->c_cells.x2s(interior), k = pmb->cells.x3s(interior), ck = pmb->c_cells.x3s(interior);
    for (int n=sn; n<=en; ++n) {
      pco->CellVolume(k,j,si,ei,fvol_[0][0]);
      for (int ci=csi; ci<=cei; ci++) {
        int i = (ci - pmb->c_cells.x1s(interior))*2 + pmb->cells.x1s(interior);
...
void MeshRefinement::RestrictFieldX1(
  ...
  MeshBlock *pmb = pmy_block_;
  auto &pco = pmb->pcoord;
  int si = (csi - pmb->c_cells.x1s(interior))*2 + pmb->cells.x1s(interior), ei = (cei - pmb->c_cells.x1s(interior))*2 + pmb->cells.x1s(interior);
  ...
  if (pmb->block_size.nx3>1) { // 3D
    ...
    for (int ci=csi; ci<=cei; ci++) {
      int i = (ci - pmb->c_cells.x1s(interior))*2 + pmb->cells.x1s(interior);
      coarse(csk,csj,ci) = fine(pmb->cells.x3s(interior),pmb->cells.x2s(interior),i);
    }
  }
  ...
  if (pmb->block_size.nx3>1) { // 3D
    ...
    for (int cj=csj; cj<=cej; cj++) {
      ...
      for (int ci=csi; ci<=cei; ci++) {
        int i = (ci - pmb->c_cells.x1s(interior))*2 + pmb->cells.x1s(interior);
        Real tarea = sarea_x2_[0][0](i) + sarea_x2_[0][0](i+1);
        coarse(pmb->c_cells.x3s(interior),cj,ci) =
            (fine(k,j,i)*sarea_x2_[0][0](i) + fine(k,j,i+1)*sarea_x2_[0][0](i+1))/tarea;
      }
    }
  } else { // 1D
    int k = pmb->cells.x3s(interior), j = pmb->cells.x2s(interior);
    pco->Face2Area(k, j, si, ei, sarea_x2_[0][0]);
    for (int ci=csi; ci<=cei; ci++) {
      int i = (ci - pmb->c_cells.x1s(interior))*2 + pmb->cells.x1s(interior);
      Real tarea = sarea_x2_[0][0](i) + sarea_x2_[0][0](i+1);
      coarse(pmb->c_cells.x3s(interior),pmb->c_cells.x2s(interior),ci) =
          (fine(k,j,i)*sarea_x2_[0][0](i) + fine(k,j,i+1)*sarea_x2_[0][0](i+1))/tarea;
    }
  }
...
void MeshRefinement::ProlongateCellCenteredValues(
  ...
  if (pmb->block_size.nx3 > 1) {
    for (int n=sn; n<=en; n++) {
      for (int k=sk; k<=ek; k++) {
        int fk = (k - pmb->c_cells.x3s(interior))*2 + pmb->cells.x3s(interior);
        ...
        Real dx3fp =  fx3p - x3c;
        for (int j = sj; j<=ej; j++) {
          int fj = (j - pmb->c_cells.x2s(interior))*2 + pmb->cells.x2s(interior);
          ...
          Real dx2fp = fx2p - x2c;
          for (int i=si; i<=ei; i++) {
            int fi = (i - pmb->c_cells.x1s(interior))*2 + pmb->cells.x1s(interior);
    ...
    int k = pmb->c_cells.x3s(interior), fk = pmb->cells.x3s(interior);
    for (int n=sn; n<=en; n++) {
      for (int j=sj; j<=ej; j++) {
        int fj = (j - pmb->c_cells.x2s(interior))*2 + pmb->cells.x2s(interior);
        const Real& x2m = pcoarsec->x2v(j-1);
    ...
    int k = pmb->c_cells.x3s(interior), fk = pmb->cells.x3s(interior), j = pmb->c_cells.x2s(interior), fj = pmb->cells.x2s(interior);
    for (int n=sn; n<=en; n++) {
      for (int i=si; i<=ei; i++) {
        int fi = (i - pmb->c_cells.x1s(interior))*2 + pmb->cells.x1s(interior);
        const Real& x1m = pcoarsec->x1v(i-1);
...
void MeshRefinement::ProlongateInternalField(
  ...
  MeshBlock *pmb = pmy_block_;
  auto &pco = pmb->pcoord;
  int fsi = (si - pmb->c_cells.x1s(interior))*2 + pmb->cells.x1s(interior), fei = (ei - pmb->c_cells.x1s(interior))*2 + pmb->cells.x1s(interior) + 1;
  if (pmb->block_size.nx3 > 1) {
    for (int k=sk; k<=ek; k++) {
      int fk = (k - pmb->c_cells.x3s(interior))*2 + pmb->cells.x3s(interior);
      for (int j=sj; j<=ej; j++) {
