...
class MeshBlockPack {
 public:
  ...
  KOKKOS_FORCEINLINE_FUNCTION
  auto &operator()(const int block, TopologicalElement el, const int n) const { return v_(block)(el, n); }
  KOKKOS_FORCEINLINE_FUNCTION
  ...
  KOKKOS_FORCEINLINE_FUNCTION
  auto &operator()(const int block, TopologicalElement el, const int n, const int k, const int j,
                   const int i) const {
...
using MapToMeshBlockVarFluxPack =
    std::map<vpack_types::UidVecPair, PackAndIndexMap<MeshBlockVarFluxPack<T>>>;
