...
void Mesh::SetMeshBlockNeighbors(
  ...
  for (auto &pmb : block_list) {
    ...
    for (const auto &nloc : neighbors) {
      ...
      // Set neighbor block ownership (note that this is ownership in coordinates of the neighbor block)
      auto &nb = all_neighbors.back();
      auto neighbor_neighbors = forest.FindNeighbors(nloc.global_loc, grid_id);
      nb.ownership = DetermineOwnership(nloc.global_loc, neighbor_neighbors, newly_refined);
