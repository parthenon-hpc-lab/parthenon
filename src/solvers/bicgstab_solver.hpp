 ...
 private:
  ...
  TaskID CreateTaskList(const TaskID &begin, const int i, TaskRegion &tr,
    ...
    auto MatVec = [this](auto &task_list, const TaskID &init_depend,
      ...
      auto send = task_list.AddTask(precom2, parthenon::SendBoundaryBuffers, spmd);
      auto recv = task_list.AddTask(precom, parthenon::ReceiveBoundaryBuffers, spmd);
      auto setb = task_list.AddTask(recv, parthenon::SetBoundaries, spmd);
      auto prolong = task_list.AddTask(setb, parthenon::ProlongateBoundaries, spmd);
      auto postcomm = prolong;
      if (this->user_postcomm_scale) {
        postcomm =
            task_list.AddTask(prolong, this->user_postcomm_scale, spmd.get(), vec_name);
      }
      ...
      if (this->user_MatVec) {
        ...
        if (this->user_pre_fluxcor) {
          auto calc_flx = task_list.AddTask(prolong, this->user_pre_fluxcor, spmd.get(),
        ...
        update_rhs = task_list.AddTask(setb, &Solver_t::MatVec<MD_t>, this, spmd.get(),
                                       name_in, name_out);
      }
      return update_rhs;
    };
    ...
    if (report && (converged || stop)) {
      if (Globals::my_rank == 0) {
        std::cout << " its= " << bicgstab_cntr << " rho= " << rhoi_old
                  ...
                  << " absolute-res0: " << global_res0.val << " relerr-tol: " << error_tol
                  << std::endl;
      }
    }
