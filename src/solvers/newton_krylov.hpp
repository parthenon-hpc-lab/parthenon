...
class NewtonKrylov : NewtonKrylov_Counter {
 public:
  ...
  std::string label() {
    return "newton_" + linear_solver->label();
  }
  std::vector<std::string> SolverState() {
    return {sol_name};
  }
  ...
  template <typename T>
  TaskID GetResidual(const TaskID &dep, T &tasks, const int i, RegionCounter &reg, TaskRegion &tr, DataType *md) {
    // zero residual
    TaskID none(0);
    auto zres = (i == 0 ? tasks.AddTask(none, [](Real *val) {
                            *val = 0.0;
                            return TaskStatus::complete;
                          }, &l2res.val)
    ...
    auto start_global_res = 
          (i == 0
              ? tasks.AddTask(new_res, &AllReduce<Real>::StartReduce, &l2res, MPI_SUM)
              : new_res);
    auto finish_global_res = tasks.AddTask(start_global_res, &AllReduce<Real>::CheckReduce, &l2res);
  ...
  TaskID createTaskList(TaskID &begin, const int i,
    ...
    auto lin_tasks = solver.AddTask(none, &NewtonKrylov<LinSolverType,DataType>::AddKrylovTasks, this,
                                    get_jac | get_res0,
                                    i, &tr, &lsolver, md, mdelta);
