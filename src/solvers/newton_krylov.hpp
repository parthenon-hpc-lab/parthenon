 ...
 public:
  ...
  TaskStatus AddSearchTasks(const TaskID &begin, const int &i, TaskRegion *tr,
    ...
    auto update = ls->AddTask(begin, &NewtonKrylov<LinSolverType, DataType>::Update, this,
                              md.get(), mdelta.get());
    // share \Delta x
    auto send = ls->AddTask(update, parthenon::SendBoundaryBuffers, md);
    auto recv = ls->AddTask(none, parthenon::ReceiveBoundaryBuffers, md);
    auto setb = ls->AddTask(recv | update, parthenon::SetBoundaries, md);
