...
class TridiagSolver : public SolverBase {
  ...
  std::string container_100, container_010, container_001,
              container_100_out, container_010_out, container_001_out,
              container_Aup, container_Adi, container_Alo, container_r;
 ...
 public:
  TridiagSolver(const std::string &container_base, const std::string &container_u,
      ...
      : container_base(container_base), container_u(container_u),
        container_rhs(container_rhs), iter_counter(0),
        eqs_(eq_in) {
    FieldTL::IterateTypes(
        [this](auto t) { this->sol_fields.push_back(decltype(t)::name()); });
    PARTHENON_REQUIRE(sol_fields.size() == 1, "Tridiagonal solver only works for a single field on a single one-dimensional block.");
  ...
  static TaskStatus SetMasks(const std::shared_ptr<MeshData<Real>> &md_100, 
    ...
    PARTHENON_REQUIRE(md_100->GetMeshPointer()->nbtotal == 1, "Tridiag only works for a single block.");
    ...
    parthenon::par_for(
      DEFAULT_LOOP_PATTERN, "SetMasks", DevExecSpace(), 0, pack_100.GetNBlocks() - 1, kb.s,
  ...
  static TaskStatus SetDiagonals(const std::shared_ptr<MeshData<Real>> &md_100_out, 
    ...
    PARTHENON_REQUIRE(md_100_out->GetMeshPointer()->nbtotal == 1, "Tridiag only works for a single block.");
    PARTHENON_REQUIRE(jb.s == jb.e, "Must be one dimensional.");
    PARTHENON_REQUIRE(kb.s == kb.e, "Must be one dimensional.");
    
    static auto desc = parthenon::MakePackDescriptorFromTypeList<FieldTL>(md_100_out.get());
    ...
    parthenon::par_for(
      DEFAULT_LOOP_PATTERN, "SetDiagonalsBasedOnMasks", DevExecSpace(), 0, pack_100.GetNBlocks() - 1,
      ...
      KOKKOS_LAMBDA(const int b, const int k, const int j, const int i) {
        ...
        printf("row %i: (%e, %e, %e) \n", i, pack_lo(b, 0, k, j, i), pack_di(b, 0, k, j, i), pack_up(b, 0, k, j, i));
    });
  ...
  static TaskStatus Solve(const std::shared_ptr<MeshData<Real>> &md_Alo,
    ...
    PARTHENON_REQUIRE(md_Alo->GetMeshPointer()->nbtotal == 1, "Tridiag only works for a single block.");
    PARTHENON_REQUIRE(jb.s == jb.e, "Must be one dimensional.");
    PARTHENON_REQUIRE(kb.s == kb.e, "Must be one dimensional.");
    ...
    // Since this needs to be sequential, we launch an outer loop of size one. Obviously this would 
    // be really inefficient on device 
    parthenon::par_for(
      ...
      KOKKOS_LAMBDA(const int) {
        ...
        for (int i = ib.s; i <= ib.e; ++i) {
          pack_di(b, 0, k, j, i) -= pack_lo(b, 0, k, j, i) * pack_up(b, 0, k, j, i - 1) / pack_di(b, 0, k, j, i - 1);  
          pack_rhs(b, 0, k, j, i) -= pack_lo(b, 0, k, j, i) * pack_rhs(b, 0, k, j, i - 1) / pack_di(b, 0, k, j, i - 1);  
          printf("[%i] (%e %e %e) x = %e\n", i, pack_lo(b, 0, k, j, i), pack_di(b, 0, k, j, i), pack_up(b, 0, k, j, i), pack_rhs(b, 0, k, j, i));
        }
        printf("\n");
        pack_out(b, 0, k, j, ib.e + 1) = 0.0; 
        for (int i = ib.e; i >= ib.s; --i) { 
          pack_out(b, 0, k, j, i) = (pack_rhs(b, 0, k, j, i)
                                   - pack_up(b, 0, k, j, i) * pack_out(b, 0, k, j, i + 1)) / pack_di(b, 0, k, j, i);
          printf("[%i] (%e %e %e) %e = %e\n", i, pack_lo(b, 0, k, j, i), pack_di(b, 0, k, j, i), pack_up(b, 0, k, j, i), pack_out(b, 0, k, j, i), pack_rhs(b, 0, k, j, i));
        }
  ...
  static TaskStatus PrintSolution(const std::shared_ptr<MeshData<Real>> &md_r,
    ...
    PARTHENON_REQUIRE(md_r->GetMeshPointer()->nbtotal == 1, "Tridiag only works for a single block.");
    PARTHENON_REQUIRE(jb.s == jb.e, "Must be one dimensional.");
    PARTHENON_REQUIRE(kb.s == kb.e, "Must be one dimensional.");
    ...
    parthenon::par_for(
      DEFAULT_LOOP_PATTERN, "SetDiagonalsBasedOnMasks", DevExecSpace(), 0, 0,
      kb.s, kb.e, jb.s, jb.e, ib.s, ib.e,
      KOKKOS_LAMBDA(const int b, const int k, const int j, const int i) {
  ...
  TaskID AddTasks(TaskList &tl, TaskID dependence, const int partition, Mesh *pmesh) {
    ...
    auto &md_100_out = pmesh->mesh_data.Add(container_100_out, partitions[partition], sol_fields);
    auto &md_010_out = pmesh->mesh_data.Add(container_010_out, partitions[partition], sol_fields);
    auto &md_001_out = pmesh->mesh_data.Add(container_001_out, partitions[partition], sol_fields);
    ...
    auto explicit_A = tl.AddTask(Ax001, SetDiagonals,
                                 md_100_out, md_010_out, md_001_out,
