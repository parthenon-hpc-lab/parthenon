...
class SwarmDeviceContext {
 public:
  ...
  void Xtoijk(const Real &x, const Real &y, const Real &z, int &i, int &j, int &k) const {
    i = static_cast<int>(
            std::floor((x - x_min_) / coords_.cellWidth(CoordinateDirection::X1DIR))) +
        ib_s_;
    j = (ndim_ > 1) ? static_cast<int>(std::floor(
                          (y - y_min_) / coords_.cellWidth(CoordinateDirection::X2DIR))) +
                          jb_s_
                    : jb_s_;
    k = (ndim_ > 2) ? static_cast<int>(std::floor(
                          (z - z_min_) / coords_.cellWidth(CoordinateDirection::X3DIR))) +
                          kb_s_
                    : kb_s_;
  }
