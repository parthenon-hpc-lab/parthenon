  ...
  void list() {
    ...
    for (auto &x : myParams_) {
      std::cout << "   " << x.first << ":" << x.second.get() << ":"
                << x.second->address() << ":" << myTypes_[x.first] << std::endl;
    }
    std::cout << std::endl;
  }
...
private:
  ...
  struct base_t {
    virtual ~base_t() =
        default; // for whatever reason I need a virtual destructor
    virtual const void *address() {
      return nullptr;
    } // for listing and debugging
  };

  template <typename T> struct object_t : base_t {
  ...
  template <typename T> void typeCheck(const std::string key, bool die) {
    // check on return type
    if (myTypes_[key].compare(std::string(typeid(T).name()))) {
      std::cout << "WRONG TYPE FOR KEY '" << key << "'" << std::endl;
      if (die)
        throw std::invalid_argument("Wrong type sent to Get<T>()");
    }
  }
