...
void CellVariable<T>::AllocateData() {
  ...
  data =
      ParArrayND<T, CellVariableState>(label(), dims_[5], dims_[4], dims_[3], dims_[2], dims_[1], dims_[0]);
...
void CellVariable<T>::AllocateFluxesAndCoarse(std::weak_ptr<MeshBlock> wpmb) {
  ...
  if (IsSet(Metadata::WithFluxes)) {
    ...
    int n_outer = 1 + (GetDim(2) > 1) * (1 + (GetDim(3) > 1));
    // allocate fluxes
    flux_data_ = ParArray7D<T, CellVariableState>(base_name + ".flux_data", n_outer, GetDim(6), GetDim(5),
  ...
  if (IsSet(Metadata::FillGhost) || IsSet(Metadata::Independent)) {
    ...
    if (pmb->pmy_mesh != nullptr && pmb->pmy_mesh->multilevel) {
      coarse_s = ParArrayND<T, CellVariableState>(base_name + ".coarse", coarse_dims_[5], coarse_dims_[4],
