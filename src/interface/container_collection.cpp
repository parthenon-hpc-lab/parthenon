...
template <typename T>
std::shared_ptr<Container<T>> &ContainerCollection<T>::Get(const std::string &label) {
  auto it = FindIterator(label);
  return it->second;
}

template <typename T>
void ContainerCollection<T>::Swap(const std::string &label1, const std::string &label2) {
