...
template <typename T>
void MeshData<T>::Initialize(BlockList_t blocks, Mesh *pmesh, int ndim, std::optional<int> gmg_level) {
...
template <typename T>
void MeshData<T>::Initialize(BlockList_t blocks, Mesh *pmesh, std::optional<int> gmg_level) {
  int ndim;
  if (pmesh != nullptr) {
    ndim = pmesh->ndim;
  }
