...
void HelicitySliceOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin, SimTime *tm,
  ...
  const auto output_mode =
      pin->GetOrAddString(output_params.block_name, "output_mode", "slice");
  ...
  par_for(
      "HelSlice_PackB", 0, pm->GetNumMeshBlocksThisRank() - 1, kb.s, kb.e, jb.s, jb.e,
  ...
  PARTHENON_REQUIRE_THROWS((std::int64_t)(ib.e - ib.s + 1) * (jb.e - jb.s + 1) *
                                   (kb.e - kb.s + 1) ==
  ...
  parthenon::par_for(
      ...
      KOKKOS_LAMBDA(const int kz_idx, const int ky_idx, const int kx_idx) {
        ...
        A_hat_kk[idx + fft_size_outbox] =
            imag_unit * (kz_phys * Bx_k - kx_phys * Bz_k) / k2;
        A_hat_kk[idx + 2 * fft_size_outbox] =
            imag_unit * (kx_phys * By_k - ky_phys * Bx_k) / k2;
      });
  ...
  for (int i = 0; i < 3; i++) {
    FFTManager->Backward(reinterpret_cast<std::complex<Real> *>(A_hat.data()) +
                             i * fft_size_outbox,
  ...
  if (full_output) {
    ...
    openPMD::Extent global_extent = {static_cast<uint64_t>(Nz), static_cast<uint64_t>(Ny),
    ...
    for (int kk = 0; kk < local_nz; kk++) {
      for (int jj = 0; jj < local_ny; jj++) {
        for (int ii = 0; ii < local_nx; ii++) {
          const std::int64_t idx =
              (std::int64_t)kk * local_ny * local_nx + (std::int64_t)jj * local_nx + ii;
    ...
    PARTHENON_REQUIRE_THROWS(chunk_offset[0] + chunk_extent[0] <= global_extent[0] &&
    ...
    const bool rank_owns_midplane =
        (kz_mid_global >= realbox.low[2] && kz_mid_global <= realbox.high[2]);

    PARTHENON_REQUIRE_THROWS((std::int64_t)local_nx * local_ny <=
