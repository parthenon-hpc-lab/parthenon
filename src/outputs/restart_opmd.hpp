...
class RestartReaderOPMD : public RestartReader {
 ...
 public:
  ...
  void ReadSwarmVar(const std::string &swarmname, const std::string &varname,
    ...
    // Backwards compatibility: Old files have position/x,y,z while new files have
    // position/x1,x2,x3 Check if the new component names exist, if not try the old ones
