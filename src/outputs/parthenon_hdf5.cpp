...
void PHDF5Output::WriteOutputFile(Mesh *pm, ParameterInput *pin, SimTime *tm) {
  ...
  WriteRestartFile(pm, pin, tm);
  return;
}
//----------------------------------------------------------------------------------------
//! \fn void PHDF5Output:::WriteRestartFile(Mesh *pm, ParameterInput *pin, bool flag)
//  \brief Cycles over all MeshBlocks and writes OutputData in the Parthenon Restart HDF5
//  format, one file per output using parallel IO.
void PHDF5Output::WriteRestartFile(Mesh *pm, ParameterInput *pin, SimTime *tm) {
  ...
  //
  // It is expected that on restart global block ID will determine which data set is read.
  //
  MeshBlock *pmb = pm->pblock;
  ...
  filename.append(".");
  std::stringstream file_number;
  file_number << std::setw(5) << std::setfill('0') << output_params.restart_file_number;
  filename.append(file_number.str());
  ...
  // advance output parameters
  output_params.restart_file_number++;
  output_params.next_restart += output_params.dt_restart;
  pin->SetInteger(output_params.block_name, "restart_file_number",
                  output_params.restart_file_number);
  pin->SetReal(output_params.block_name, "next_restart", output_params.next_restart);
  return;
}
