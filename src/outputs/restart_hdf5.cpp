...
#ifdef ENABLE_HDF5
namespace {
herr_t CollectRootDatasets(hid_t group, const char *name, const H5L_info_t *,
                           void *data) {
  hid_t dataset = -1;
  ...
  if (dataset >= 0) {
    static const std::set<std::string> non_field_datasets{
        "SparseInfo", "SparseDeallocCount", "VolumeLocations", "Levels",
        "LogicalLocations"};
...
void RestartReaderHDF5::ReadBlocks(const std::string &name, IndexRange range,
  ...
  const IndexDomain file_domain =
      has_ghost != 0 ? IndexDomain::entire : IndexDomain::interior;
  ...
  PARTHENON_REQUIRE_THROWS(hdl.rank == file_total_dim,
                           "Dataset '" + name + "' has rank " + std::to_string(hdl.rank) +
                               " but registered field expects rank " +
                               std::to_string(file_total_dim));
  for (int d = 1; d < file_total_dim; ++d) {
    PARTHENON_REQUIRE_THROWS(hdl.dims[d] == file_count[d],
                             "Dataset '" + name + "' dimension " + std::to_string(d) +
                                 " is " + std::to_string(hdl.dims[d]) +
                                 " but registered field expects " +
