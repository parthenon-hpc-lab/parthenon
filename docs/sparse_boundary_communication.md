...
*Note that setting `do_sparse_allocation = true` minimizes the memory allocated for sparse variables but may result in substantially slower MPI communication since `MPI_Irecv` can't be posted until the incoming message size is known. There is a flag, `Metadata::SparseCommunication`, that can be set for variables to make them use this memory minimizing communication pattern.* 
