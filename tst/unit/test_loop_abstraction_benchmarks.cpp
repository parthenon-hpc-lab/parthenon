...
constexpr std::string_view PatternName() {
  ...
  } else if constexpr (LOOP_TAG == loop_tag::bvoi && INNER_TAG == inner_tag::logical_coords) {
    return "bvoi/logical_coords";
  } else if constexpr (LOOP_TAG == loop_tag::bvoi && INNER_TAG == inner_tag::memory) {
    return "bvoi/memory";
  } else if constexpr (LOOP_TAG == loop_tag::bovi && INNER_TAG == inner_tag::logical_flat) {
    return "bovi/logical_flat";
  } else if constexpr (LOOP_TAG == loop_tag::bovi && INNER_TAG == inner_tag::logical_coords) {
    return "bovi/logical_coords";
  } else if constexpr (LOOP_TAG == loop_tag::bovi && INNER_TAG == inner_tag::memory) {
    return "bovi/memory";
  } else if constexpr (LOOP_TAG == loop_tag::boiv && INNER_TAG == inner_tag::logical_flat) {
    return "boiv/logical_flat";
  } else if constexpr (LOOP_TAG == loop_tag::boiv && INNER_TAG == inner_tag::logical_coords) {
...
KOKKOS_INLINE_FUNCTION Real EncodeValue(const int b, const int v, const int k, const int j,
                                        const int i) {
...
template <class IndexSpaceType>
void CheckParity(const parthenon::HostArray5D<Real> &lhs, const parthenon::HostArray5D<Real> &rhs,
...
std::vector<int> NinnerCases(const int logical_cells) {
  std::vector<int> cases{1, std::max(1, logical_cells - 1), logical_cells, logical_cells + 1};
...
constexpr std::array<ProblemSpec, 3> CoverageSpecs() {
  return {ProblemSpec{2, 3, 2, 2, 1}, ProblemSpec{1, 1, 1, 1, 1}, ProblemSpec{2, 4, 3, 2, 2}};
}
...
parthenon::HostArray5D<Real> RunKjiBody(const ProblemSpec &spec, const int ninner,
  ...
  if (use_kokkos) {
    ...
    plb2::loop_abstraction::outer(idx_space, [&](const auto &idx_range, int b) {
      for (int v = 0; v < kNVars; ++v) {
        plb2::loop_abstraction::inner(idx_range, [&](const int k, const int j, const int i) {
          out(b, v, k, j, i) += EncodeValue(b, v, k, j, i);
        });
      }
    });
...
void RunContractCase(const ProblemSpec &spec, const int ninner, const char *body_name,
  ...
  const auto default_out = kji_body ? RunKjiBody<LOOP_TAG, INNER_TAG>(spec, ninner, false)
                                    : RunAutoIndexBody<LOOP_TAG, INNER_TAG>(spec, ninner, false);
  const auto kokkos_out = kji_body ? RunKjiBody<LOOP_TAG, INNER_TAG>(spec, ninner, true)
                                   : RunAutoIndexBody<LOOP_TAG, INNER_TAG>(spec, ninner, true);

  CheckParity(default_out, kokkos_out,
              PatternIndexSpace<LOOP_TAG, INNER_TAG>(spec.nblocks, spec.nx, spec.ny, spec.nz,
                                                    spec.nghost, ninner));

  CheckLogicalContract(
      PatternIndexSpace<LOOP_TAG, INNER_TAG>(spec.nblocks, spec.nx, spec.ny, spec.nz,
                                            spec.nghost, ninner),
      default_out);
}
