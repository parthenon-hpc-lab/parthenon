...
struct TaskChecker {
  ...
  std::vector<std::set<std::size_t>>
      dag_dependencies; // Stores dependencies for each task
  std::vector<std::vector<std::size_t>>
      region_tasks; // Maps region index to list of tasks in that region
  ...
  // Qualifier: Handles task qualification properties, particularly synchronization
  // requirements
  struct Qualifier {
    ...
    void Resolve() {
      if (pqualifier.LocalSync()) {
        ...
        for (auto task : task_ids)
          combined_dependencies.insert(tc->dag_dependencies[task].begin(),
                                       tc->dag_dependencies[task].end());
        ...
        for (std::size_t task = 0; task < tc->current_global_task_id; ++task) {
          ...
          // If it depends on the sync group, make it depend on all tasks in the combined
          // dependencies
          if (depends_on)
  ...
  auto Execute() { return tc.Execute(); }
  ...
  std::size_t AddTask(std::size_t region, std::size_t task_list,
    ...
    auto id_out = tl.AddTask(
        qualifier.pqualifier, tid,
        [&](std::size_t task_id, TaskChecker *task_checker) {
          bool all_dependencies_complete{true};
          for (auto &task : task_checker->dag_dependencies[task_id]) {
            all_dependencies_complete =
                all_dependencies_complete && task_checker->task_complete[task];
          }
