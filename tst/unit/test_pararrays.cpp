...
KOKKOS_FORCEINLINE_FUNCTION void many_array_kernel(
    const Array& arr0, const Array& arr1, const Array& arr2, const Array& arr3, const Array& arr4,
    const Array& arr5, const Array& arr6, const Array& arr7, const Array& arr8, const Array& arr9,
    const Array& arr_out, const int k, const int j, const int i){
...
TEST_CASE("Check many arrays","[ParArrayND]") {
  ...
  for( int run = 0; run < nruns; run++){
    parthenon::par_for(parthenon::loop_pattern_flatrange_tag,
                       ...
                       exec_space, 0, N-1, 0, N-1, 0, N-1,
                       KOKKOS_LAMBDA(const int k, const int j, const int i) {
                         many_array_kernel(raw0,raw1,raw2,raw3,raw4,raw5,raw6,raw7,raw8,raw9,raw_out,k,j,i);
                       });
  ...
  for( int run = 0; run < nruns; run++){
    parthenon::par_for(parthenon::loop_pattern_flatrange_tag,
                       ...
                       exec_space, 0, N-1, 0, N-1, 0, N-1,
                       KOKKOS_LAMBDA(const int k, const int j, const int i) {
                         many_array_kernel(nda0,nda1,nda2,nda3,nda4,nda5,nda6,nda7,nda8,nda9,nda_out,k,j,i);
                       });
  }
  Kokkos::fence();
  timer.reset();
  ...
  for( int run = 0; run < nruns; run++){
    parthenon::par_for(parthenon::loop_pattern_flatrange_tag,
                       ...
                       exec_space, 0, N-1, 0, N-1, 0, N-1,
                       KOKKOS_LAMBDA(const int k, const int j, const int i) {
                         many_array_kernel(xtra0,xtra1,xtra2,xtra3,xtra4,xtra5,xtra6,xtra7,xtra8,xtra9,xtra_out,k,j,i);
                       });
  }
  Kokkos::fence();
  timer.reset();
