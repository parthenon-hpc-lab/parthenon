...
TEST_CASE("HDF5 analysis reads field interiors with precision conversion",
  ...
  const std::string filename = "restart_reader_analysis_data_test.h5";
  {
    ...
    const H5D dataset = H5D::FromHIDCheck(H5Dcreate2(
        file, "density", H5T_NATIVE_FLOAT, dataspace, H5P_DEFAULT, H5P_DEFAULT,
        H5P_DEFAULT));
    const float values[] = {-1.0F, 2.0F, 3.0F, -4.0F};
    PARTHENON_HDF5_CHECK(H5Dwrite(dataset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL,
                                 H5P_DEFAULT, values));
  }
  ...
  const OutputUtils::VarInfo variable_info(
      "density", "density", "", {}, 1, dimensions, metadata,
      {TopologicalElement::CC}, false, false, IndexShape(2, 1));
