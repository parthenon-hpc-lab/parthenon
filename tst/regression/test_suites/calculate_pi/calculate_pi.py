...
import utils.parthenon as parthenon          # utilities for running Athena++
...
def run(driver, input_file):
    ...
    print("Checking driver and input files")
    print(driver)
    print(input_file)
    #    is_valid_file(driver)
    print("driver valid")
    #is_valid_file(input_file)
    print("input_file valid")
    ...
    # the athinput file.)
    # Need to pass the executable
    print("Calling Parthenon run")
    parthenon.run(driver,input_file, arguments)
    print("Parthenon run done")
    #parthenon.run('','hydro_sr/athinput.mb_1', arguments)
...
def analyze():
    ...
    #x_ref, _, _, data_ref = parthenon_read.vtk('data/sr_hydro_shock1_hlle.vtk')
    ...
    print("Error abs")
    error_abs_e = math.fabs( math.pi - pi_val ) / math.pi  #comparison.l1_diff(x_ref, e_ref, x_new, e_new)
    ...
    #    analyze_status = False
    #jif error_rel_mx > 0.01 or np.isnan(error_rel_mx):
     #   analyze_status = False
    print("analyze status")
    print(analyze_status)
