...
std::shared_ptr<StateDescriptor> Initialize(ParameterInput *pin) {
  ...
  PARTHENON_REQUIRE_THROWS(num_scalars > 0, "Burgers benchmark requires num_scalars >= 1");
...
Real EstimateTimestepMesh(MeshData<Real> *md) {
  ...
  Real min_dt;
  parthenon::par_reduce(parthenon::loop_pattern_mdrange_tag, "burgers::EstimateTimestep",
    ...
    KOKKOS_LAMBDA(const int b, const int k, const int j, const int i, Real &ldt) {
      ...
      ldt = std::min(ldt, 1.0/( (std::abs(v(b,0,k,j,i)))/coords.Dx(X1DIR,k,j,i)
...
TaskStatus CalculateFluxes(MeshData<Real> *md) {
  ...
  size_t scratch_size = 0;
  constexpr int scratch_level = 0;
  parthenon::par_for_outer(DEFAULT_OUTER_LOOP_PATTERN, "burgers::reconstruction",
    DevExecSpace(), scratch_size, scratch_level, 0, nblocks-1,
    ...
    KOKKOS_LAMBDA(team_mbr_t member, const int b, const int k, const int j) {
      ...
      for (int n = iu_lo; n <= iu_hi; n++) {
        ...
        if (xrec) {
          ...
          recon_loop(ib.s-1, ib.e+1, pqm2, pqm1, pq, pqp1, pqp2, pql, pqr);
        }
        if (yrec) {
          Real *pqm2 = &v(b, n, k, j-2, 0);
          Real *pqm1 = &v(b, n, k, j-1, 0);
          Real *pqp1 = &v(b, n, k, j+1, 0);
          Real *pqp2 = &v(b, n, k, j+2, 0);
          Real *pql = &v(b, iuly_lo + n, k, j+1, 0);
          Real *pqr = &v(b, iury_lo + n, k, j, 0);
          recon_loop(ib.s, ib.e, pqm2, pqm1, pq, pqp1, pqp2, pql, pqr);
        }
        if (zrec) {
          Real *pqm2 = &v(b, n, k-2, j, 0);
          Real *pqm1 = &v(b, n, k-1, j, 0);
          Real *pqp1 = &v(b, n, k+1, j, 0);
          Real *pqp2 = &v(b, n, k+2, j, 0);
  ...
  parthenon::par_for_outer(DEFAULT_OUTER_LOOP_PATTERN, "burgers::reconstruction",
    ...
    KOKKOS_LAMBDA(team_mbr_t member, const int b, const int k, const int j) {
      ...
      if (xflux) {
        ...
        uflux_loop(ib.s, ib.e+1, uxl, uxr, uyl, uyr, uzl, uzr, uxl, uxr, sl, sr,
                  fxux, fxuy, fxuz);
        member.team_barrier();
      ...
      if (yflux) {
        ...
        uflux_loop(ib.s, ib.e, uxl, uxr, uyl, uyr, uzl, uzr, uyl, uyr, sl, sr,
                   fyux, fyuy, fyuz);
        member.team_barrier();
        for (int n = 3; n <= iu_hi; n++) {
