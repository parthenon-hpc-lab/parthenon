...
inline RaggedMetadata BuildRaggedMetadata(int blocks, int variables, int active_min,
                                          int active_max) {
  ...
  for (int block = 0; block < blocks; ++block) {
    ...
    metadata.active_counts[block] =
        clamped_min + static_cast<int>(hash % static_cast<std::uint32_t>(span));
  }
  return metadata;
}

inline int ActiveVariablesForBlock(const RaggedMetadata &metadata, bool ragged, int block,
                                   int fallback) {
  if (!ragged) {
    return fallback;
  }
