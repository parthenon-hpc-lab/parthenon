...
int SelectNvarsForBlock(const Dataset &dataset, int block) {
...
template <typename Body>
void RunCpuBoivContiguous(const Dataset &dataset, int logical_inner_size, Body body) {
  const auto &spec = dataset.problem;
  const auto &data = dataset.data;
...
void RunCpuBoviContiguous(const Dataset &dataset, int logical_inner_size, AccessBuilder build_access,
                          Body body) {
  const auto &spec = dataset.problem;
  const auto &data = dataset.data;
  const auto logical_indexer = spec.logical_indexer;
...
template <typename Body>
void RunCpuBoivLogical(const Dataset &dataset, int logical_inner_size, Body body) {
  const auto &spec = dataset.problem;
  const auto &data = dataset.data;
  const auto logical_indexer = spec.logical_indexer;
...
template <typename AccessBuilder, typename Body>
void RunCpuBvoiContiguous(const Dataset &dataset, int logical_inner_size,
...
template <typename Body>
void RunCpuBoviLogical(const Dataset &dataset, int logical_inner_size, Body body) {
  const auto &spec = dataset.problem;
  const auto &data = dataset.data;
  const auto logical_indexer = spec.logical_indexer;
...
template <typename Body>
void RunCpuBvoiLogical(const Dataset &dataset, int logical_inner_size, Body body) {
  const auto &spec = dataset.problem;
  const auto &data = dataset.data;
  const auto logical_indexer = spec.logical_indexer;
...
template <typename AccessBuilder, typename Body>
void RunKokkosBoviTeamContiguous(const Dataset &dataset, int logical_inner_size,
                                 AccessBuilder build_access, Body body) {
  const auto &spec = dataset.problem;
  const auto &data = dataset.data;
  const auto logical_indexer = spec.logical_indexer;
...
template <typename Body>
void RunKokkosBoviTeamLogical(const Dataset &dataset, int logical_inner_size, Body body) {
  const auto &spec = dataset.problem;
  const auto &data = dataset.data;
  const auto logical_indexer = spec.logical_indexer;
  const int cells_per_block = static_cast<int>(logical_indexer.size()) / spec.nblocks;
